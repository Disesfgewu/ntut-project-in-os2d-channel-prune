...
class LCP:
    def __init__(self, net, aux_net=None, dataloader=None, img_normalization=None, gen_dataloader=None):
        ...
        self._gen_dataloader = gen_dataloader
    ...
    def _get_layer_by_name(self, layer_name, net):
        ...
        try:
            ...
            return None
    def _get_feature_stats_from_tensor(self, img_tensor, layer_name):
        """
        [新函式] 從一個圖像張量計算特徵統計，這是為了生成數據而設計的。
        """
        try:
            # 確保張量在 GPU 上
            ...
            print(f"[ERROR] 從 Tensor 計算特徵統計時失敗: {e}")
            return None
    
    # ===================================================================
    #  請用這段程式碼，完整替換您 lcp.py 中的同名函式
    # ===================================================================
    def compute_channel_importance_no_grad(self, layer_name, lambda_rate=1.0, use_image_num=None, random_seed=None, use_generative=False, gen_weight=0.3):
        """
        基於數學推導的無梯度通道重要性計算（重構後的混合模式）。
        """
        self._net = self._net.cuda()
        self._prune_net = self._prune_net.cuda()
        ...
        # --- 1. 處理真實數據 (與您原始程式碼完全相同的流程) ---
        print(f"[LCP] 開始處理真實數據...")
        real_image_ids = list(map(int, self._get_db().get_image_ids()))
        if use_image_num is not None and use_image_num < len(real_image_ids):
            real_image_ids = random.sample(real_image_ids, use_image_num)
        
        # 移除您用於測試的固定 ID 行，或將其保留並註解掉
        # real_image_ids = [ 61 , 62 , 63 ] # Test for Debug

        real_feature_stats = []
        all_reconstruction_errors = []
        all_auxiliary_losses = []
        
        print(f"從真實資料集抽樣 {len(real_image_ids)} 張影像: {real_image_ids}")
        for image_id in real_image_ids:
            # 呼叫舊的、依賴 image_id 的函式，確保原始功能正常
            feature_stats = self._extract_and_compute_theoretical_statistics(image_id, layer_name)
            if feature_stats:
                real_feature_stats.append(feature_stats)
            print( f"Image ID {image_id} 特徵 {feature_stats}" )
        ...
        if use_generative:
            print(f"\n[LCP] 開始處理生成數據...")
            if self._gen_dataloader is None:
                raise ValueError("gen_dataloader 未初始化，無法使用生成數據。")

            # 從 gen_dataloader 中抽樣
            num_gen_images_to_sample = use_image_num if use_image_num is not None else len(self._gen_dataloader.dataset)
            gen_image_tensors = []
            data_iter = iter(self._gen_dataloader)
            
            print(f"從 gen_dataloader 抽樣 {num_gen_images_to_sample} 張探測影像...")
            while len(gen_image_tensors) < num_gen_images_to_sample:
                try:
                    batch = next(data_iter)
                    for img_tensor in batch:
                        gen_image_tensors.append(img_tensor.unsqueeze(0))
                        if len(gen_image_tensors) >= num_gen_images_to_sample:
                            break
                except StopIteration:
                    print("警告：gen_dataloader 已遍歷完畢。")
                    break
            
            # 使用新的輔助函式計算特徵統計
            for img_tensor in gen_image_tensors:
                stats = self._get_feature_stats_from_tensor(img_tensor, layer_name)
                if stats:
                    gen_feature_stats.append(stats)
                    print( f"Gen Image 特徵 {stats}" )
    ...
    #  請用這段程式碼，完整替換您 lcp.py 中的同名函式
    # ===================================================================
    def _integrate_importance_with_mathematical_formula(self, real_feature_stats, reconstruction_errors, auxiliary_losses, lambda_rate, gen_feature_stats=None, gen_weight=0.5):
        """基於數學推導整合最終重要性分數（支援加權）"""
        if not real_feature_stats:
            # 如果沒有真實數據的統計，就無法計算，直接報錯
            if not gen_feature_stats:
                raise ValueError("沒有任何有效的特徵統計資料可以進行整合。")
            # 如果只有生成數據，則直接使用生成數據（權重設為1）
            print("[LCP] 僅使用生成數據進行整合。")
            real_feature_stats = gen_feature_stats
            gen_feature_stats = None

        num_channels = len(real_feature_stats[0]['channels'])
        
        # 計算真實數據的基礎統計重要性
        real_base_importance = np.zeros(num_channels)
        for i in range(num_channels):
            channel_importances = [stats['channels'][i]['importance'] for stats in real_feature_stats if stats and 'channels' in stats]
            if channel_importances:
                real_base_importance[i] = np.mean(channel_importances)

        # 計算生成數據的基礎統計重要性 (如果存在)
        gen_base_importance = np.zeros(num_channels)
        if gen_feature_stats:
            for i in range(num_channels):
                channel_importances = [stats['channels'][i]['importance'] for stats in gen_feature_stats if stats and 'channels' in stats]
                if channel_importances:
                    gen_base_importance[i] = np.mean(channel_importances)
        
        # --- 加權計算 ---
        if gen_feature_stats and gen_weight > 0 and len(gen_feature_stats) > 0:
            print(f"[LCP] 整合重要性：真實數據權重 {1-gen_weight:.2f}, 生成數據權重 {gen_weight:.2f}")
            base_importance = (1 - gen_weight) * real_base_importance + gen_weight * gen_base_importance
        else:
            print("[LCP] 整合重要性：僅使用真實數據")
            base_importance = real_base_importance

        # 損失貢獻項 (永遠只基於真實數據計算)
        reconstruction_contribution = np.mean(reconstruction_errors) if reconstruction_errors else 0
        auxiliary_contribution = np.mean(auxiliary_losses) if auxiliary_losses else 0
        
        # 最終重要性
        # 調整貢獻項的影響力，避免其量級遠大於 base_importance
        final_importance = base_importance + (reconstruction_contribution * 0.1) + (auxiliary_contribution * lambda_rate * 0.1)
    ...
    def get_channel_selection_and_write_to_db( self, layer_name , discard_rate=0.5, use_image_num=3, random_seed=42, use_generative=False, gen_weight=0.5):
        """
    ...
    def prune_layer(self, layer_name, discard_rate=0.5, use_image_num=3, random_seed=42, use_generative=False, gen_weight=0.5):
    ...
    def debug_for_test_vision(self, dataloader_train, img_normalization, box_coder, cfg, count=1):
        ...
        self._prune_net = self._prune_net.cuda()
        get, labels, scores = generate_detection_boxes(dataloader_train, self._prune_net, img_normalization, box_coder, image_id, class_id, cfg, class_num=count*2, debug_or_eval=True)
    ...
    def evaluate(self, dataloader_train, img_normalization, box_coder, cfg, count=50, iou_threshold=0.5, eval_list = None):
        """
        評估剪枝網路在訓練集上的性能
            ...
            for eval_idx in range(count):
                try:
                    ...
                    if eval_list == None:
                        ...
                        available_batch_idx = random.choice(list(map(int, eval_list )))
                        image_id = dataloader_train.get_image_ids_for_batch_index(available_batch_idx)[0]
                        batch = dataloader_train.get_batch(available_batch_idx)
                    ...
                    get, labels, scores = generate_detection_boxes(
                        ...
                        image_id, class_id, cfg, class_num=len(t_points[class_id]), debug_or_eval=True
                    )
