...
def show_detections(boxes, image_to_show,
                    cfg_visualization,
                    class_ids=None, image_id=None, show_fig=True, ifdetection=False, path=None):
    ...
    return show_annotated_image(img=image_to_show,
                               boxes=boxes,
                               ...
                               showfig=show_fig,
                               image_id=image_id, 
                               ifdetection=ifdetection)


def show_gt_boxes(image_id, gt_boxes, class_ids, dataloader, image_to_show=None, show_img=True, path=None):
...
def show_annotated_image(img, boxes, labels, scores, class_ids, score_threshold=0.0,
                         ...
                         max_dets=None, showfig=False, image_id=None, path=None, ifdetection=False):
    
    good_ids = torch.nonzero(scores.float() > score_threshold).view(-1)
    
    # 用於存儲最終選中的 ID（需要考慮 default_boxes 和 transform_corners）
    final_good_ids = good_ids.clone()
    
    if good_ids.numel() > 0:
        # 每個 label 只保留得分最高的一個檢測結果
        selected_ids = []
        detected_labels = labels[good_ids]
        unique_labels = detected_labels.unique()
        
        for ul in unique_labels:
            # 找出屬於當前 label 的所有檢測結果
            ul_mask = detected_labels == ul
            ul_ids = good_ids[ul_mask]
            
            if len(ul_ids) > 0:
                # 取該 label 中 score 最高的檢測結果
                ul_scores = scores[ul_ids]
                max_score_idx = ul_scores.argmax()
                selected_ids.append(ul_ids[max_score_idx])
        
        # 轉換為 tensor
        if len(selected_ids) > 0:
            selected_ids = torch.stack(selected_ids)
            
            # 如果設定了 max_dets，從所有選中的結果中取總分最高的前 max_dets 個
            if max_dets is not None and len(selected_ids) > max_dets:
                selected_scores = scores[selected_ids]
                _, order = selected_scores.sort(descending=True)
                selected_ids = selected_ids[order[:max_dets]]
            
            # 更新 final_good_ids 為最終選中的 ID
            final_good_ids = selected_ids
            
            boxes = boxes[selected_ids].cpu()
            labels = labels[selected_ids].cpu()
            scores = scores[selected_ids].cpu()
            ...
            label_names = []
            box_colors = []
            final_good_ids = torch.LongTensor([])  # 空的 tensor
    else:
        # 沒有符合閾值的檢測結果
        boxes = BoxList.create_empty(boxes.image_size)
        labels = torch.LongTensor(0)
    ...
    # 處理 default_boxes（使用篩選後的 ID）
    if default_boxes is not None and final_good_ids.numel() > 0:
        default_boxes = default_boxes[final_good_ids].cpu()
        
        # 合併 default_boxes 和檢測結果
        ...
        # 如果沒有 default_boxes 或沒有檢測結果，只使用檢測結果的 bbox
        if hasattr(boxes, 'bbox_xyxy'):
            boxes = boxes.bbox_xyxy
        # 如果 boxes 是空的，保持原狀

    # 處理 transform_corners（使用篩選後的 ID）
    if transform_corners is not None and final_good_ids.numel() > 0:
        transform_corners = transform_corners[final_good_ids].cpu()
    else:
...
def vis_image(img, boxes=None, label_names=None, scores=None, colors=None, image_id=None, polygons=None,showfig=False, path=None, ifdetection=False):
    ...
    if ifdetection:
        ...
        bgr_img = np.array(pil_img)[..., ::-1]  # RGB to BGR (OpenCV)
        info = {
            "boxes": boxes,
            "label_names": label_names,
            "scores": scores,
    ...
    if showfig:
        ...
        if path is None:
            path = ""
        plt.savefig(f"./visualized_images/image_{image_id}_{path}.png", bbox_inches='tight', pad_inches=0.1)
    plt.close()
    return fig
